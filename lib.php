...
/**
 * Saves a new instance of the mod_multiprogress into the database.
 *
...
function multiprogress_add_instance($moduleinstance, $mform = null) {
    ...
    $id = $DB->insert_record('multiprogress', $moduleinstance);

    return $id;
}

/**
 * Updates an instance of the mod_multiprogress in the database.
 *
...
function multiprogress_update_instance($moduleinstance, $mform = null) {
    ...
    return $DB->update_record('multiprogress', $moduleinstance);
}

/**
 * Removes an instance of the mod_multiprogress from the database.
 *
...
function multiprogress_delete_instance($id) {
    ...
    $DB->delete_records('multiprogress', ['id' => $id]);

    return true;
}

/**
 * Is a given scale used by the instance of mod_multiprogress?
 *
 * This function returns if a scale is being used by one mod_multiprogress
...
/**
 * Checks if scale is being used by any instance of mod_multiprogress.
 *
...
/**
 * Creates or updates grade item for the given mod_multiprogress instance.
 *
...
function multiprogress_grade_item_update($moduleinstance, $reset=false) {
    ...
    grade_update('/mod/multiprogress', $moduleinstance->course, 'mod', 'mod_multiprogress', $moduleinstance->id, 0, null, $item);
}

/**
 * Delete grade item for given mod_multiprogress instance.
 *
...
/**
 * Update mod_multiprogress grades in the gradebook.
 *
 * Needed by {@see grade_update_mod_grades()}.
 *
 * @param stdClass $moduleinstance Instance object with extra cmidnumber and modname property.
...
function multiprogress_update_grades($moduleinstance, $userid = 0) {
    ...
    $grades = [];
    grade_update('/mod/multiprogress', $moduleinstance->course, 'mod', 'mod_multiprogress', $moduleinstance->id, 0, $grades);
}
...
/**
 * File browsing support for mod_multiprogress file areas.
 *
 * @package     mod_multiprogress
 * @category    files
 *
...
/**
 * Serves the files from the mod_multiprogress file areas.
 *
 * @package     mod_multiprogress
 * @category    files
 *
 ...
 * @param stdClass $context The mod_multiprogress's context.
...
/**
 * Extends the global navigation tree by adding mod_multiprogress nodes if there is a relevant content.
 *
 * This can be called by an AJAX request so do not rely on $PAGE as it might not be set up properly.
 *
 * @param navigation_node $multiprogressnode An object representing the navigation tree node.
 * @param stdClass $course
 ...
 *
 * This function is called when the context for the page is a mod_multiprogress module.
